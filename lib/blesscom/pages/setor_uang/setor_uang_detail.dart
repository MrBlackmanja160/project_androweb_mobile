...
class SetorUangDetail extends StatefulWidget {
  final dynamic infoLog;
  final dynamic master;

  const SetorUangDetail({
  ...
  @override
  _SetorUangDetailState createState() => _SetorUangDetailState();
}

class _SetorUangDetailState extends State<SetorUangDetail> {
  ...
  bool _loadingSavePayment = false;
  String _initialPhoto = "";
  // ignore: unused_field
  final bool _showSelect = true;
  var random = math.Random();

  final TextEditingController _conBayar = TextEditingController();
  ...
  Future<Iterable<Map<String, dynamic>>> _generateData() async {
    ...
    if (response.success) {
      ...
      for (var element in data) {
        ...
        // Sum grand total
        _grandTotal +=
            double.tryParse(element['total'].toString().replaceAll(".", "")) ??
                0;
      }
  ...
  void _deleteMaster() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Yakin Hapus Bukti Setor Uang?"),
        actions: [
          ElevatedButton(
            onPressed: () {
  ...
  void _uploadPhoto(String value) async {
    ...
    try {
      ...
      request.fields["transaksi"] = "serahuang";
      request.files.add(
  ...
  void _savePayment() async {
    if (mounted) {
      setState(() {
        _loadingSavePayment = true;
      });
    }

    HttpPostResponse response = await Helper.sendHttpPost(
      urlSetorUangSetorManual,
      postData: {
        "iduser": await Helper.getPrefs("iduser"),
        "token": await Helper.getPrefs("token"),
        "temp_id": widget.master["temp_id"].toString(),
  ...
  void initState() {
    ...
    _conBayar.text = int.tryParse(
            widget.master["setoranmanual"].toString().replaceAll(".", ""))
        .toString();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: SizedBox(
          height: Helper.getScreenHeight(context) - kToolbarHeight + 250,
          child: Column(
            children: [
              Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      Padding(
                        ...
                        child: InputText(
                          label: "Bayar Manual",
                          controller: _conBayar,
                          keyboardType: TextInputType.number,
                          inputFormatters: [
                            FilteringTextInputFormatter.digitsOnly
                          ],
                          readOnly: !(_editable && !_loadingPosting),
                          enabled: _editable && !_loadingPosting,
                        ),
                      ),
                    ],
              ...
              Padding(
                ...
                child: ElevatedButton(
                  onPressed:
                      _editable && !_loadingSavePayment && !_loadingPosting
                          ? _savePayment
                          : null,
                  child: _loadingSavePayment
                      ? Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: const [
                            SizedBox(
              ...
              Expanded(
                ...
                child: Container(
                  child: OkeTable(
                    ...
                    onTabRow: widget.master["status"] == "O" ? _onTabRow : null,
                    onSort: (value) {
                      setState(() => _isLoading = true);
