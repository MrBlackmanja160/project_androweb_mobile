...
import 'package:kalbemd/blesscom/pages/input_penjualan_sales/input_penjualan_sales_adddetail.dart';
import 'package:kalbemd/blesscom/pages/input_po/input_po_adddetail.dart';
...
class InputPenjualanSalesExplore extends StatefulWidget {
  final dynamic infoLog;
  final dynamic master;

  const InputPenjualanSalesExplore({
  ...
  @override
  _InputPenjualanSalesExploreState createState() =>
      _InputPenjualanSalesExploreState();
}

class _InputPenjualanSalesExploreState
    extends State<InputPenjualanSalesExplore> {
  ...
  void _addItem() {
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => InputPenjualanSalesAddDetail(
              master: widget.master,
              queryAddPenjualanProduk: widget.infoLog["infoLog"]
                  ["query_add_penjualanproduk"],
            ),
          ),
        )
  ...
  void _deleteItemConfirm(dynamic data) async {
    HttpPostResponse response = await Helper.sendHttpPost(
      urlInputPenjualanSalesDetailDelete,
      postData: {
        "iduser": await Helper.getPrefs("iduser"),
        "token": await Helper.getPrefs("token"),
        "temp_id": widget.master["temp_id"].toString(),
  ...
  void _deleteMaster() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Yakin Hapus Bukti Input Penjualan Sales?"),
        actions: [
          ElevatedButton(
            onPressed: () {
  ...
  void _uploadPhoto(String value) async {
    ...
    try {
      ...
      request.fields["transaksi"] = "inputpenjualansales";
      request.files.add(
