...
class LogProduk extends StatefulWidget {
  const LogProduk({Key? key}) : super(key: key);

  @override
  _LogProdukState createState() => _LogProdukState();
}

class _LogProdukState extends State<LogProduk> {
  ...
  Future<Iterable<Map<String, dynamic>>> _generateData() async {
    ...
    if (response.success) {
      ...
      _hargaPerPcs =
          double.tryParse(_infoBarang["hargaperpcs"].toString()) ?? 0;
