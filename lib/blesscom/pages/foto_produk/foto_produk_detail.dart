...
import 'package:kalbemd/blesscom/forms/input_photo_keterangan.dart';
import 'package:kalbemd/blesscom/pages/foto_produk/foto_produk_add.dart';
import 'package:kalbemd/blesscom/pages/foto_produk/model_jenis_foto.dart';
...
class FotoProdukDetail extends StatefulWidget {
  final dynamic infoLog;
  final dynamic master;

  const FotoProdukDetail({
  ...
  @override
  _FotoProdukDetailState createState() => _FotoProdukDetailState();
}

class _FotoProdukDetailState extends State<FotoProdukDetail> {
  ...
  void _deleteMaster() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Yakin Hapus Bukti Foto Activity Motoris?"),
        actions: [
          ElevatedButton(
            onPressed: () {
  ...
  void _uploadPhotoProduk(String value, String idjenisphoto) async {
    ...
    try {
      ...
      final Uri uri = Uri.parse(urlUploadPhotoProduk);
  ...
  List<ModelJenisFoto> datafoto = [];
  List<ModelJenisFoto> dataJenisFoto = [];

  Future<void> loadDataJenisFoto(String tempId) async {
    // Request data
    HttpPostResponse response = await Helper.sendHttpPost(
      urlMasterJenisFotoMtr,
      postData: {
        "iduser": await Helper.getPrefs("iduser"),
        "token": await Helper.getPrefs("token"),
